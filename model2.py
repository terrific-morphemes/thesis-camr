...
class Model():
    ...
    def _set_rel_tag_codebooks(self, instances, parser):
        ...
        for i, inst in enumerate(instances):
            ...
            for g, d in gold_graph.tuples():
                if isinstance(g, int):
                    gnode = gold_nodes[g]
                    g_span_wds = [tok['form'] for tok in sent_tokens if tok['id'] in range(
                        gnode.start, gnode.end)]
                    if gnode.start == gnode.end and isinstance(gnode.start, int):
                        gnode.end = gnode.start + 1
                    ...
                    if g_span_ne not in ['O', 'NUMBER']:  # is name entity
                        self.token_to_concept_table[g_span_ne].add(
                            g_entity_tag)
                    if g_span_ne == 'O':  # record unk templates
                ...
                if isinstance(d, int):
                    dnode = gold_nodes[d]
                    if dnode.start == dnode.end and isinstance(dnode.start, int):
                        dnode.end = dnode.start + 1
                        #import pdb;pdb.set_trace()
                    d_span_wds = [tok['form'] for tok in sent_tokens
                                  if tok['id'] in range(dnode.start,
                                                        dnode.end)]
    ...
    def _set_rel_tag_codebooks_oracle(self, instance, parser):
        '''
        set up relation and concept tag base on gold graph
        '''

        state = Parser.State.init_state(instance, parser.verbose)
        ref_graph = state.gold_graph
        sent_tokens = instance.tokens
        ...
        while not state.is_terminal():
            ...
            if state.is_permissible(action):
                ...
                if 'tag' in action:
                    concept_tag = action['tag']

                    if not isinstance(state.idx, int):  # abt action
                        self.tag_codebook['ABTTag'].add(concept_tag)
                        self.abttag_count[concept_tag] += 1
                    else:  # next2

                        cur_idx = state.idx
                        cur_node = ref_graph.nodes[cur_idx] if cur_idx in ref_graph.nodes else state.A.nodes[cur_idx]
                        span_wds = [tok['form'] for tok in sent_tokens if tok['id'] in range(
                            cur_node.start, cur_node.end)]
                        span_ne = sent_tokens[cur_idx]['ne']

                        if span_ne not in ['O', 'NUMBER']:  # is name entity
                            self.token_to_concept_table[span_ne].add(
                                concept_tag)
                        self.token_to_concept_table[','.join(
                            span_wds)].add(concept_tag)
                        if isinstance(concept_tag, ETag):
                            self.tag_codebook['ETag'].add(concept_tag)
                        elif isinstance(concept_tag, ConstTag):
                            self.tag_codebook['ConstTag'].add(concept_tag)
                        else:
                            self.tag_codebook['Concept'].add(concept_tag)
                elif 'edge_label' in action:
                    edge_label = action['edge_label']
                    self.rel_codebook.add(edge_label)
                state = state.apply(action)

            else:
                raise Exception('Impermissibe action: %s' % (action))
    ...
    def save_model(self, model_filename):
        ...
        print >> self.elog, 'class size: %s \nrelation size: %s \ntag size: %s' % (self.class_codebook.size(
        ), self.rel_codebook.size(), map(lambda x: '%s->%s ' % (x, self.tag_codebook[x].size()), self.tag_codebook.keys()))
        print >> self.elog, 'feature codebook size: %s' % (
            ','.join(('%s:%s') % (i, f.size()) for i, f in self.feature_codebook.items()))
        # print 'weight shape: %s' % (self.avg_weight.shape)
        print >> self.elog, 'weight shape: %s' % (
            ','.join(('%s:%s') % (i, w.shape) for i, w in enumerate(self.avg_weight)))
        print >> self.elog, 'token to concept table: %s' % (
