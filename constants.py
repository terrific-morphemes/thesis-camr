...
FLAG_RNE = False
FLAG_VERB = False
FLAG_DEPPARSER = 'stdconv+charniak'
FLAG_ONTO = 'wsj'
FLAG_FHAR = True
...
ABT_REL = '_ABT_REL_'
ABT_PCPT = '_ABT_PCPT_'
ABT_TOKEN = {'form': ABT_FORM, 'lemma': ABT_LEMMA, 'pos': ABT_POS,
             'ne': ABT_NE, 'rel': ABT_REL, 'cpt_label_pred': ABT_PCPT}
...
START_PCPT = '_START_PCPT_'
START_TOKEN = {'form': START_FORM, 'lemma': START_LEMMA, 'pos': START_POS,
               'ne': START_NE, 'rel': START_REL, 'cpt_label_pred': START_PCPT}
...
DET_T2G_ORACLE_ABT_ALIGN = 4
DETERMINE_STRING_TO_GRAPH_ORACLE = None

ACTION_WITH_TAG = set([INFER, ADDCHILD])
...
FEATS_ABBR = {}
FEATS_ABBR['w'] = 'form'
FEATS_ABBR['lemma'] = 'form'  # for Chinese
FEATS_ABBR['t'] = 'pos'
FEATS_ABBR['tag'] = 'tag'
FEATS_ABBR['ne'] = 'ne'
FEATS_ABBR['dl'] = 'rel'
...
FEATS_ABBR['cptpred'] = 'cpt_label_pred'
FEATS_ABBR['hyphen'] = 'hyphen'
FEATS_ABBR['pcptispred'] = 'pcpt_ispred'
FEATS_ABBR['pcptsense'] = 'pcpt_sense'
FEATS_ABBR['eqpcpt'] = 'eqpcpt'
FEATS_ABBR['pcptpart1'] = 'pcpt_part1'
...
# '''
# DEFAULT_RULE_FILE = os.path.join(CURDIR,'rules/dep2amrLabelRules')
...
# '''
# DEFAULT_NOM_FILE = os.path.join(CURDIR,'./resources/nombank-dict.1.0')
...
# DEFAULT_BROWN_CLUSTER = os.path.join(CURDIR, './resources/wclusters-engiga')
...
# PATH_TO_VERB_LIST = os.path.join(CURDIR, './resources/verbalization-list-v1.01.txt')
...
# PATH_TO_MORPH_LIST = os.path.join(CURDIR, './resources/morph-verbalization-v1.01.txt')
# def _load_morph_list(path_to_file):
#     morphdict = defaultdict(list)
#     seg_pattern = '::DERIV-[^\s:]*'
#     with open(path_to_file, 'r') as f:
#         for line in f:
#             line = line.strip()
#             value = None
#             for i,item in enumerate(re.split(seg_pattern, line)):
#                 if i == 0:
#                     value = item.strip()[1:-1]
#                 else:
#                     key = item.strip()[1:-1]
#                     morphdict[key].append(key)

#     return morphdict

# MORPH_LIST = _load_morph_list(PATH_TO_MORPH_LIST)


# PATH_TO_COUNTRY_LIST=os.path.join(CURDIR,'./resources/country-list.csv')
