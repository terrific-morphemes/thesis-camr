...
class SpanGraph(object):
    ...
    def init_ref_graph(amr, alignment, sent=None):
        ...
        if amr.roots[0] in span_alignment:
            ...
            if debuglevel > 1:
                print >> log, "GraphID:%s WARNING:root %s not aligned!" % (
                    SpanGraph.graphID, amr.node_to_concepts[amr.roots[0]])
                            ...
                            if second_and < max and second_and > min and first_and < smax and first_and > smin:
                                ...
                                for sc in tmp_children:
                                    if sc != first_and:
                                        ...
                                        self.add_edge(
                                            first_and, sc, edge_label)

    @staticmethod
    def init_ref_graph_abt(amr, alignment, s2c_alignment, sent=None):
        """Instantiate graph from AMR graph and alignment"""

        def init_unaligned_var(amr, h, unaligned_vars,
                               spgraph, span_alignment):
            hconcept = amr.node_to_concepts[h] if h in amr.node_to_concepts else h

            h_index = None
            hspan = None
            hnode = None
            if 'name' in amr[h]:  # name entity
                d = amr[h]['name'][0]
                literals = [v[0] for k, v in amr[d].items()]
                hconcept = '%s|name@name|name' % (hconcept)
                aligned_literals = [l for l in literals if l in span_alignment]
                if len(aligned_literals) == 0:
                    h_index = h
                else:
                    aligned_literal = aligned_literals[0]
                    h_index = span_alignment[aligned_literal][0].start
                    hspan = Span(h_index, h_index,
                                 aligned_literal, ETag(hconcept))
                    span_alignment[h].append(hspan)
                    unaligned_vars.add(aligned_literal)
                h_node = SpanNode(h_index, h_index, literals, ETag(hconcept))
                if h_index not in spgraph.nodes:
                    spgraph.add_node(h_node)
                unaligned_vars.add(h)
                unaligned_vars.add(d)

            # elif hconcept == 'have-org-role-91' and 'ARG2' in amr[h]:
            #     d = amr[h]['ARG2'][0]

            #     dconcept = amr.node_to_concepts[d]
            #     hconcept = '%s|ARG2@%s' % (hconcept, dconcept)

            #     h_index = d if d not in span_alignment else span_alignment[d][0].start
            #     h_node = SpanNode(h_index, h_index, [dconcept], ETag(hconcept))
            #     hspan = Span(h_index, h_index, [dconcept], ETag(hconcept))
            #     span_alignment[h].append(hspan)
            #     if h_index not in spgraph.nodes:
            #         spgraph.add_node(h_node)
            #     unaligned_vars.add(h)
        ...
        for h in varables:
            # ignore unaligned constants and vars already handled
            ...
            if not h in span_alignment:  # unaligned concept
                h_index, h_node, hspan = init_unaligned_var(
                    amr, h, unaligned_vars, spgraph, span_alignment)
                # continue
            else:
                hspans = span_alignment[h]
                if len(hspans) < 2:
                    ...
                    hspan = hspans.pop(0)
                if hspan.start not in spgraph.nodes:
                    h_node = SpanNode.from_span(hspan)
                    spgraph.add_node(h_node)
                    if hconcept == 'and':
                        and_nodes.add(h_node.start)  # aligned 'and' nodes
                h_index = hspan.start

            for edge, ds in amr[h].items():
                d = ds[0]
                if d in unaligned_vars:
                    continue

                if edge == 'wiki':
                    continue

                if isinstance(d, (Polarity, Interrogative)) and d not in span_alignment:
                    # handle polarity and interrogative;
                    spgraph.nodes[h_index].tag = ETag(hconcept + '|' + d)
                    unaligned_vars.add(d)
                    continue

                if (d not in amr.node_to_concepts and d not in span_alignment) or d in unaligned_vars:
                    continue
                ...
                if not d in span_alignment:  # unaligned concept
                    ...
                    d_index, d_node, dspan = init_unaligned_var(amr, d,
                                                                unaligned_vars,
                                                                spgraph, span_alignment)
                else:
                    dspans = span_alignment[d]
        ...
        if amr.roots[0] in span_alignment:
            hchild_id = span_alignment[amr.roots[0]][0].start
            spgraph.add_edge(0, hchild_id, constants.NULL_EDGE)
        else:
            if debuglevel > 1:
                print >> log, "GraphID:%s WARNING:root %s not aligned!" % (
                    SpanGraph.graphID, amr.node_to_concepts[amr.roots[0]])
    ...
    def new_abt_node(self, gov_index, tag, reverse=False):
        ...
        abt_node = SpanNode(abt_node_index, abt_node_index,
                            [constants.ABT_FORM], tag)
    ...
    def merge_node(self, idx1, idx2, ext_wds=True):
    ...
    def get_possible_parent_constrained(self, currentIdx, currentChildIdx, i=2):
        ...
        if isinstance(currentChildIdx, int) and 'pred' in self.sent[currentChildIdx]:
            candidate_parents = candidate_parents.union(set(
                prd for prd in self.sent[currentChildIdx]['pred'] if prd != currentChildIdx and prd in self.nodes and prd not in cur.parents and prd not in cur.children))
        return candidate_parents
