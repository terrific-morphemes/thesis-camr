...
class AMR(defaultdict):
    ...
    def parse_string(cls, amr_string, RENAME_NODE=False):
        ...
        lex_rules = [
            ...
            ("LITERAL", "'[^\s(),]+"),
            ("INTERROGATIVE",
             "\s(interrogative|imperative|expressive)(?=[\s\)])"),
    ...
    def get_variable(self, posID):
        """return variable given postition ID; JAMR style"""
        reent_var = None
        seq = self.dfs()[0]
        for node in seq:
            if node.seqID == posID:
    ...
    def get_match(self, subgraph):
        ...
        for v in self.node_to_concepts:
            if v[0] == subroot[0] and self.node_to_concepts[v] == subroot:
                ...
                if rcp is not None:
                    return v, concepts_on_the_path + rcp
    ...
    def bfs(self):
        ...
        for i, r in enumerate(self.roots):
            seqID = str(i)
            # node, incoming edge and depth
            queue = deque([((r,), None, 0, seqID)])
    ...
    def print_triples(self):
        ...
        for rel, parent, child in amr_triples:
            if not isinstance(child, (Quantity, Polarity, Interrogative, StrLiteral)):
                result += "%s(%s,%s)\n" % (rel,
                                           self.node_to_concepts[parent], self.node_to_concepts[child])
            else:
                result += "%s(%s,%s)\n" % (rel,
                                           self.node_to_concepts[parent], child)
        return result
