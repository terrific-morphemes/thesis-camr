...
from common.span_graph2 import SpanGraph
from common.amr_graph2 import *
import numpy as np
...
class GraphState(object):
    ...
    def init_state(instance, verbose=0):
        ...
        if verbose > 1:
            print >> sys.stderr, "Sentence ID:%s, initial sigma:%s" % (
                GraphState.sentID, sigma)
    ...
    def _init_atomics(self):
        ...
        sp1 = GraphState.sent[self.A.nodes[self.idx].parents[0]
                              ] if self.A.nodes[self.idx].parents else NOT_ASSIGNED
        ...
        if sp1 != NOT_ASSIGNED and len(self.A.nodes[sp1['id']].children) > 1:
            ...
            slsb = GraphState.sent[children[idx_order - 1]
                                   ] if idx_order > 0 else NOT_ASSIGNED
            srsb = GraphState.sent[children[idx_order + 1]
                                   ] if idx_order < len(children) - 1 else NOT_ASSIGNED
            sr2sb = GraphState.sent[children[idx_order + 2]
                                    ] if idx_order < len(children) - 2 else NOT_ASSIGNED
        else:
            slsb = EMPTY
            srsb = EMPTY
            sr2sb = EMPTY
    ...
    def get_possible_actions(self, train):
        ...
        if isinstance(currentIdx, int):
            current_tok_lemma = ','.join(tok['form'] for tok in GraphState.sent if tok['id'] in range(
                currentNode.start, currentNode.end))
            current_tok_form = ','.join(tok['form'] for tok in GraphState.sent if tok['id'] in range(
                currentNode.start, currentNode.end))
            current_tok_ne = GraphState.sent[currentIdx]['ne']
        else:
            current_tok_form = ABT_TOKEN['form']
            # if currentIdx != START_ID else START_TOKEN['lemma']
            current_tok_lemma = ABT_TOKEN['form']
            # if currentIdx != START_ID else START_TOKEN['ne']
            current_tok_ne = ABT_TOKEN['ne']
        ...
        if currentChildIdx:  # beta not empty
            ...
            if currentIdx != 0:  # not root
                if not currentChild.SWAPPED:
                    ...
                    actions.extend([{'type': REATTACH, 'parent_to_attach': p}
                                    for p in currentGraph.get_possible_parent_constrained(currentIdx, currentChildIdx)])
                ...
                actions.extend({'type': REENTRANCE, 'parent_to_add': x}
                               for x in currentGraph.get_possible_reentrance_constrained(currentIdx, currentChildIdx))
            else:
            ...
            if current_tok_lemma in token_to_concept_table:
                all_candidate_tags.extend(
                    list(token_to_concept_table[current_tok_lemma]))
                ...
                all_candidate_tags.extend(
                    list(token_to_concept_table[current_tok_ne]))
                # all_candidate_tags.append(current_tok_lemma.lower())
                # all_candidate_tags.extend(GraphState.model.tag_codebook['ETag'].labels())
            elif current_tok_lemma == ABT_TOKEN['form']:
                # all_candidate_tags.extend(tag_codebook['ABTTag'].labels())
                pass
                # all_candidate_tags.extend(currentGraph.nodes[currentIdx].tag)
            else:
                original_form = current_tok_lemma.lower()
            ...
            if isinstance(currentIdx, int) and 'frmset' in GraphState.sent[currentIdx] \
               and GraphState.sent[currentIdx]['frmset'] not in all_candidate_tags:
                all_candidate_tags.append(
                    GraphState.sent[currentIdx]['frmset'])

            if not currentNode.children and currentIdx != 0:
                actions.append({'type': DELETENODE})
            actions.append({'type': NEXT2})

            actions.extend({'type': NEXT2, 'tag': z}
                           for z in all_candidate_tags)
            # if currentNode.num_parent_infer_in_chain < 3 and currentNode.num_parent_infer == 0:
            #    actions.append({'type':ADDCHILD})
        return actions
    ...
    def get_node_context(self, idx):
        ...
        if self.A.nodes[idx].parents:
            p1 = GraphState.sent[self.A.nodes[idx].parents[0]] if isinstance(
                self.A.nodes[idx].parents[0], int) else ABT_TOKEN
            # p1_brown_repr = BROWN_CLUSTER[p1['form']]
            # p1['brown4'] = p1_brown_repr[:4] if len(
            #     p1_brown_repr) > 3 else p1_brown_repr
            # p1['brown6'] = p1_brown_repr[:6] if len(
            #     p1_brown_repr) > 5 else p1_brown_repr
            # p1['brown10'] = p1_brown_repr[:10] if len(
            #     p1_brown_repr) > 9 else p1_brown_repr
            # p1['brown20'] = p1_brown_repr[:20] if len(
            #     p1_brown_repr) > 19 else p1_brown_repr
        else:
            p1 = NOT_ASSIGNED
        ...
        if p1 != NOT_ASSIGNED and len(self.A.nodes[self.A.nodes[idx].parents[0]].children) > 1:
            ...
            if idx_order > 0:
                lsb = GraphState.sent[children[idx_order - 1]] if isinstance(
                    children[idx_order - 1], int) else ABT_TOKEN
            else:
                lsb = NOT_ASSIGNED
            if idx_order < len(children) - 1:
                rsb = GraphState.sent[children[idx_order + 1]] if isinstance(
                    children[idx_order + 1], int) else ABT_TOKEN
            else:
                rsb = NOT_ASSIGNED
            if idx_order < len(children) - 2:
                r2sb = GraphState.sent[children[idx_order + 2]] if isinstance(
                    children[idx_order + 2], int) else ABT_TOKEN
            else:
                r2sb = NOT_ASSIGNED
        else:
            lsb = EMPTY
    ...
    def get_feature_context_window(self, action):
        ...
        def delta_func(tag_to_predict, tok_form):
            if isinstance(tag_to_predict, (ConstTag, ETag)):
                ...
                if tag_lemma == tok_form:
                    return '1'
                i = 0
                slength = len(tag_lemma) if len(
                    tag_lemma) < len(tok_form) else len(tok_form)
        ...
        s0_atomics = GraphState.sent[self.idx].copy() if isinstance(
            self.idx, int) else ABT_TOKEN  # GraphState.abt_tokens[self.idx]
        #s0_brown_repr = BROWN_CLUSTER[s0_atomics['form']]
        # s0_atomics['brown4'] = s0_brown_repr[:4] if len(
        #     s0_brown_repr) > 3 else s0_brown_repr
        # s0_atomics['brown6'] = s0_brown_repr[:6] if len(
        #     s0_brown_repr) > 5 else s0_brown_repr
        # s0_atomics['brown8'] = s0_brown_repr[:8] if len(
        #     s0_brown_repr) > 7 else s0_brown_repr
        # s0_atomics['brown10'] = s0_brown_repr[:10] if len(
        #     s0_brown_repr) > 9 else s0_brown_repr
        # s0_atomics['brown20'] = s0_brown_repr[:20] if len(
        #     s0_brown_repr) > 19 else s0_brown_repr
        ...
        s0_atomics['len'] = self.A.nodes[self.idx].end - \
            self.A.nodes[self.idx].start if isinstance(
                self.idx, int) else NOT_ASSIGNED
        # s0_atomics['cap']=s0_atomics['form'].istitle()
        s0_atomics['dch'] = sorted([GraphState.sent[j]['form'].lower() if isinstance(
            j, int) else ABT_FORM for j in self.A.nodes[self.idx].del_child])
        s0_atomics['reph'] = sorted([GraphState.sent[j]['form'].lower() if isinstance(
            j, int) else ABT_FORM for j in self.A.nodes[self.idx].rep_parent])
        ...
        core_args = set([self.A.get_edge_label(self.idx, child) for child in self.A.nodes[self.idx].children if self.A.get_edge_label(
            self.idx, child).startswith('ARG') and child != self.cidx])
        ...
        # prop feature
        s0_atomics['frmset'] = GraphState.sent[self.idx]['frmset'] if isinstance(
            self.idx, int) and 'frmset' in GraphState.sent[self.idx] else NOT_ASSIGNED
        ...
        if not self.cidx:
            ...
            if 'tag' in action:  # next2
                ...
                s0_atomics['txdelta'] = delta_func(
                    tag_to_predict, s0_atomics['form'])
                ...
                if isinstance(self.idx, int):
                    ...
                    s0_atomics['pcpt_sense'] = pcpt_match.group(
                        1) if pcpt_match else NOT_ASSIGNED
                    s0_atomics['eqpcpt'] = s0_atomics['pcpt_sense'] == tag_to_predict.split(
                        '-')[-1]
                    s0_pcpt_comp = s0_pcpt.split('_')
                    s0_atomics['pcpt_part1'] = s0_pcpt_comp[0] if len(
                        s0_pcpt_comp) > 1 else NOT_ASSIGNED
                    s0_atomics['pcpt_part2'] = s0_pcpt_comp[1] if len(
                        s0_pcpt_comp) > 1 else NOT_ASSIGNED
        ...
        if self.cidx and self.cidx != START_ID:
            b0_atomics = GraphState.sent[self.cidx].copy() if isinstance(
                self.cidx, int) else ABT_TOKEN  # GraphState.abt_tokens[self.cidx]
            #b0_brown_repr = BROWN_CLUSTER[b0_atomics['form']]
            # b0_atomics['brown4'] = b0_brown_repr[:4] if len(
            #     b0_brown_repr) > 3 else b0_brown_repr
            # b0_atomics['brown6'] = b0_brown_repr[:6] if len(
            #     b0_brown_repr) > 5 else b0_brown_repr
            # b0_atomics['brown8'] = b0_brown_repr[:8] if len(
            #     b0_brown_repr) > 7 else b0_brown_repr
            # b0_atomics['brown10'] = b0_brown_repr[:10] if len(
            #     b0_brown_repr) > 9 else b0_brown_repr
            # b0_atomics['brown20'] = b0_brown_repr[:20] if len(
            #     b0_brown_repr) > 19 else b0_brown_repr
            b0_atomics['concept'] = self.A.nodes[self.cidx].tag
            bprs2, bprs1, bp1, blsb, brsb, br2sb = self.get_node_context(
                self.cidx)
            ...
            b0_atomics['reph'] = sorted([GraphState.sent[rp]['form'] if isinstance(
                rp, int) else ABT_FORM for rp in self.A.nodes[self.cidx].rep_parent])
            b0_atomics['len'] = self.A.nodes[self.cidx].end - \
                self.A.nodes[self.cidx].start if isinstance(
                    self.cidx, int) else NOT_ASSIGNED
            b0_atomics['dch'] = sorted([GraphState.sent[j]['form'].lower() if isinstance(
                j, int) else ABT_FORM for j in self.A.nodes[self.cidx].del_child])
            b0_atomics['eqne'] = (
                s0_atomics['ne'] == b0_atomics['ne'] and b0_atomics['ne'] in PRE_MERGE_NETAG)
            b0_atomics['isne'] = b0_atomics['ne'] in PRE_MERGE_NETAG
            b0_atomics['hastrace'] = len(
                self.A.nodes[self.cidx].incoming_traces) > 0
            ...
            if isinstance(self.cidx, int) and isinstance(self.idx, int):
                path, direction = GraphState.deptree.get_path(
                    self.cidx, self.idx)
                if self.A.nodes[self.idx].end - self.A.nodes[self.idx].start > 1:
                    path_pos_str = [(GraphState.sent[i]['pos'], GraphState.sent[i]['rel']) for i in path[1:-1]
                                    if i not in range(self.A.nodes[self.idx].start, self.A.nodes[self.idx].end)]
                    path_x_str_pp = [('X', 'X') if not isprep(GraphState.sent[i]) else GraphState.sent[i]['form']
                                     for i in path[1:-1] if i not in range(self.A.nodes[self.idx].start, self.A.nodes[self.idx].end)]
                else:
                    path_pos_str = [
                        (GraphState.sent[i]['pos'], GraphState.sent[i]['rel']) for i in path[1:-1]]
                    path_x_str_pp = [('X', 'X') if not isprep(
                        GraphState.sent[i]) else GraphState.sent[i]['form'] for i in path[1:-1]]
        ...
        if action['type'] in [REATTACH, REENTRANCE]:
            ...
            if parent_to_attach is not None:
                a0_atomics = GraphState.sent[parent_to_attach].copy() if isinstance(
                    parent_to_attach, int) else ABT_TOKEN  # GraphState.abt_tokens[parent_to_attach]
                # a0_brown_repr = BROWN_CLUSTER[a0_atomics['form']]
                # a0_atomics['brown4'] = a0_brown_repr[:4] if len(
                #     a0_brown_repr) > 3 else a0_brown_repr
                # a0_atomics['brown6'] = a0_brown_repr[:6] if len(
                #     a0_brown_repr) > 5 else a0_brown_repr
                # a0_atomics['brown8'] = a0_brown_repr[:8] if len(
                #     a0_brown_repr) > 7 else a0_brown_repr
                # a0_atomics['brown10'] = a0_brown_repr[:10] if len(
                #     a0_brown_repr) > 9 else a0_brown_repr
                # a0_atomics['brown20'] = a0_brown_repr[:20] if len(
                #     a0_brown_repr) > 19 else a0_brown_repr
                a0_atomics['concept'] = self.A.nodes[parent_to_attach].tag
                aprs2, aprs1, ap1, alsb, arsb, ar2sb = self.get_node_context(
                    parent_to_attach)
                ...
                tr = [t for r, t in itr]
                a0_atomics['istrace'] = parent_to_attach in tr if len(
                    tr) > 0 else EMPTY
                a0_atomics['rtr'] = itr[tr.index(
                    parent_to_attach)][0] if parent_to_attach in tr else EMPTY
                a0_atomics['hasnsubj'] = b0_atomics['rel'] in set(
                    GraphState.sent[c]['rel'] for c in self.A.nodes[parent_to_attach].children if isinstance(c, int))
                ...
                #path_x_str=[(GraphState.sent[i]['pos'],GraphState.sent[i]['rel']) for i in path[1:-1]]
                    if self.A.nodes[parent_to_attach].end - self.A.nodes[parent_to_attach].start > 1:
                        apath_x_str = [('X', 'X') for i in path[1:-1] if i not in range(
                            self.A.nodes[parent_to_attach].start, self.A.nodes[parent_to_attach].end)]
                        apath_pos_str = [(GraphState.sent[i]['pos'], GraphState.sent[i]['rel']) for i in path[1:-1]
                                         if i not in range(self.A.nodes[parent_to_attach].start, self.A.nodes[parent_to_attach].end)]
                        apath_pos_str_pp = [(GraphState.sent[i]['pos'], GraphState.sent[i]['rel']) if not isprep(GraphState.sent[i]) else GraphState.sent[i]['form']
                                            for i in path[1:-1] if i not in range(self.A.nodes[parent_to_attach].start, self.A.nodes[parent_to_attach].end)]
                    else:
                        apath_x_str = [('X', 'X') for i in path[1:-1]]
                        apath_pos_str = [
                            (GraphState.sent[i]['pos'], GraphState.sent[i]['rel']) for i in path[1:-1]]
                        apath_pos_str_pp = [(GraphState.sent[i]['pos'], GraphState.sent[i]['rel']) if not isprep(
                            GraphState.sent[i]) else GraphState.sent[i]['form'] for i in path[1:-1]]
            ...
            # path_lemma_str.append(GraphState.sent[path[-1]]['rel'])
                    ...
                    b0_atomics['apathprepwd'] = str(
                        apath_pos_str_pp) + direction
        ...
        if self.cidx == START_ID:
            ...
            s0_atomics['nech'] = len(set(GraphState.sent[j]['ne'] if isinstance(
                j, int) else ABT_NE for j in self.A.nodes[self.idx].children) & INFER_NETAG) > 0
            ...
            if self.A.nodes[self.idx].children:
                ...
                s0_atomics['c1lemma'] = GraphState.sent[c1]['form'].lower(
                ) if isinstance(c1, int) else ABT_LEMMA
                s0_atomics['c1dl'] = GraphState.sent[c1]['rel'] if isinstance(
                    c1, int) else ABT_LEMMA
            else:
    ...
    def get_gold_edge_graph(self):
        ...
        for t_tuple in parsed_tuples:
            if t_tuple in gold_tuples:
                gold_arc_label = self.gold_graph.get_edge_label(
                    t_tuple[0], t_tuple[1])
                gold_edge_graph.set_edge_label(
                    t_tuple[0], t_tuple[1], gold_arc_label)
    ...
    def get_gold_label_graph(self):
        ...
        for t_tuple in parsed_tuples:
            if t_tuple in gold_tuples:
                gold_arc_label = self.gold_graph.get_edge_label(
                    t_tuple[0], t_tuple[1])
                gold_label_graph.set_edge_label(
                    t_tuple[0], t_tuple[1], gold_arc_label)
    ...
    def evaluate(self):
        ...
        for t_tuple in parsed_tuples:
            p, c = t_tuple
            p_p, c_p = p, c
            if p in self.A.abt_node_table:
                p = self.A.abt_node_table[p]
            if c in self.A.abt_node_table:
                c = self.A.abt_node_table[c]
            if p_p not in visited_nodes:
                ...
                if p in self.gold_graph.nodes:
                    ...
                    # and not (isinstance(g_p_tag,(ETag,ConstTag)) or re.match('\w+-\d+',g_p_tag)): #and isinstance(g_p_tag,(ETag,ConstTag)):
                    if p_tag == g_p_tag:
            ...
            if c_p not in visited_nodes:
                ...
                if c in self.gold_graph.nodes:
                    ...
                    # and not (isinstance(g_c_tag,(ETag,ConstTag)) or re.match('\w+-\d+',g_c_tag)): #and isinstance(g_c_tag,(ETag,ConstTag)):
                    if c_tag == g_c_tag:
                        num_correct_tags += 1.0
                    else:
    ...
    def get_score(self, act_type, feature, train=True):
        ...
        feat_idx = map(
            GraphState.model.feature_codebook[act_idx].get_index, feature)
    ...
    def apply(self, action):
        ...
        other_params = dict(
            [(k, v) for k, v in action.items() if k != 'type' and v is not None])
    ...
    def next2(self, tag=None):
        newstate = self.pcopy()
        if tag:
            newstate.A.set_node_tag(newstate.idx, tag)
        newstate.sigma.pop()
        newstate.idx = newstate.sigma.top()
        newstate.beta = Buffer(
            newstate.A.nodes[newstate.idx].children) if newstate.idx != -1 else None
        if newstate.beta is not None:
            newstate.beta.push(START_ID)
        newstate.cidx = newstate.beta.top() if newstate.beta else None
    ...
    def reattach(self, parent_to_attach=None, edge_label=None):
        newstate = self.pcopy()
        newstate.A.reattach_node(
            newstate.idx, newstate.cidx, parent_to_attach, edge_label)
        newstate.beta.pop()
        newstate.cidx = newstate.beta.top() if newstate.beta else None
        return newstate
    ...
    def swap(self, edge_label):
        ...
        newstate.A.swap_head2(newstate.idx, newstate.cidx,
                              newstate.sigma, edge_label)
        newstate._fix_prop_feature(newstate.idx, newstate.cidx)
        #newstate.idx = newstate.cidx
        tmp = newstate.sigma.pop()
        tmp1 = newstate.sigma.pop(
        ) if newstate.A.nodes[tmp].num_parent_infer > 0 else None
        if newstate.cidx not in newstate.sigma:
            newstate.sigma.push(newstate.cidx)
        if tmp1:
            newstate.sigma.push(tmp1)
        newstate.sigma.push(tmp)

        # TODO revisit
        # newstate.beta.pop()
        newstate.beta = Buffer([c for c in newstate.A.nodes[newstate.idx].children if c !=
                                newstate.cidx and c not in newstate.A.nodes[newstate.cidx].parents])
        newstate.cidx = newstate.beta.top() if newstate.beta else None
    ...
    def add_child(self, tag):
        newstate = self.pcopy()
        abt_node_index = newstate.A.new_abt_node(
            newstate.idx, tag, reverse=True)
        tmp = newstate.sigma.pop()
        newstate.sigma.push(abt_node_index)
        newstate.sigma.push(tmp)
        newstate.idx = newstate.sigma.top()
    ...
    def _fix_prop_feature(self, idx, cidx):
        ...
        if isinstance(idx, int) and isinstance(cidx, int):
            ...
            ctok['pred'].update(
                dict((k, v) for k, v in tok.get('pred', {}).items() if k != cidx))
            ...
            ctok['args'].update(
                dict((k, v) for k, v in tok.get('args', {}).items() if k != cidx))
    ...
    def replace_head(self):
        ...
        newstate._fix_prop_feature(newstate.idx, newstate.cidx)
        if newstate.idx in newstate.sigma:
            newstate.sigma.remove(newstate.idx)
        if newstate.cidx in newstate.sigma:
            newstate.sigma.remove(newstate.cidx)  # pushing cidx to top
        newstate.sigma.push(newstate.cidx)
    ...
    def merge(self):
        ...
        if tmp1 < tmp2:
            ...
            if tmp2 in newstate.sigma:
                newstate.sigma.remove(tmp2)  # pushing tmp2 to the top
            newstate.sigma.push(tmp2)
            if tmp1 in newstate.sigma:
                newstate.sigma.remove(tmp1)
    ...
    def get_parsed_amr(span_graph):
        ...
        def unpack_node(node, amr, variable, date_set):
            node_id = node.start
            node_tag = node.tag

            core_var = None
            tokens_in_span = GraphState.sent[node.start:node.end] if isinstance(
                node_id, int) else []
            if isinstance(node_tag, ETag):
                ...
                for i, abs_tag in enumerate(node_tag.split('|')):
                    ...
                    if i == 0:  # node already initialized
                        ...
                        amr._add_triple(pre_abs_id, rel, abs_id)
                        pre_abs_id = abs_id
                    elif abs_tag in ['interrogative', 'imperative', 'expressive']:  # negation
                        abs_id = Interrogative(abs_tag)
                        foo = amr[abs_id]
                        rel = 'mode'
                        amr._add_triple(pre_abs_id, rel, abs_id)
                        pre_abs_id = abs_id
                ...
                last_abs_tag = abs_tag if abs_tag else node_tag.split('|')[
                    i - 1]
                if last_abs_tag in ['interrogative', 'imperative', 'expressive', '-', '+'] or '@' in abs_tag:
                ...
                for i, tok in enumerate(tokens_in_span):
                    ...
                    if last_abs_tag == 'name':
                        amr._add_triple(last_abs_id, 'op' +
                                        str(i + 1), StrLiteral(tok['form']))
                    elif last_abs_tag == 'date-entity':
                        date_pattern = [
                            ('d1',
                             '^({0}{0}{0}{0})(\-{0}{0})?(\-{0}{0})?$'.format('[0-9]')),
                            ('d2', '^({0}{0})({0}{0})({0}{0})$'.format(
                                '[0-9]'))
                        ]
                        date_rule = '|'.join('(?P<%s>%s)' % (p, d)
                                             for p, d in date_pattern)
                        ...
                        if m:
                            ...
                            if date_type == 'd1':
                                ...
                                if m.group(3) is not None:
                                    month = str(int(m.group(3)[1:]))
                                if m.group(4) is not None:
                                    day = str(int(m.group(4)[1:]))
                            elif date_type == 'd2':
                                year = '20' + m.group(6)
                            ...
                            foo = amr[year]
                            amr._add_triple(
                                last_abs_id, 'year', Quantity(year))
                            if month and month != '0':
                                foo = amr[month]
                                amr._add_triple(
                                    last_abs_id, 'month', Quantity(month))
                            if day and day != '0':
                                foo = amr[day]
                                amr._add_triple(
                                    last_abs_id, 'day', Quantity(day))
                        ...
                        foo = amr[new_id]
                        core_var = new_id
                        amr.node_to_concepts[new_id] = tok['form'].lower()
                        amr._add_triple(last_abs_id, rel_in_span, new_id)

                    else:
                        if re.match('[0-9\-]+', tok['form']):
                            amr._add_triple(
                                last_abs_id, rel_in_span, Quantity(tok['form']))
                        else:
                            new_id = tok['form'][0].lower()
                            j = 0
                            while new_id in amr:
        ...
        for parent, child in span_graph.tuples():
            ...
            try:
                if parent == 0:
                    if cvar not in amr:
                        cvar, ccvar = unpack_node(
                            span_graph.nodes[child], amr, cvar, date_set)
                        cpvar_cache[cvar] = ccvar
                    if cvar not in amr.roots:
                        amr.roots.append(cvar)
                else:
                    rel_label = span_graph.get_edge_label(parent, child)
                    if pvar not in amr:
                        pvar, cpvar = unpack_node(
                            span_graph.nodes[parent], amr, pvar, date_set)
                    ...
                    elif rel_label == NULL_EDGE and amr.node_to_concepts.get(cvar, None) == NULL_TAG:
                        pass
                    else:
        ...
        if len(amr.roots) > 1:
            ...
            root_list = [node_prefix + str(r)
                         for r in span_graph.get_multi_roots()]
            root_cpt_set = set(amr.node_to_concepts.get(r, r)
                               for r in root_list)
            if len(root_cpt_set) == 1:
                new_root_var = root_list[0]
                for multi_root in root_list[1:]:
                ...
                amr.roots = [FAKE_ROOT_VAR]
        # Const tag
        elif len(amr.roots) == 1 and amr.roots[0] not in amr.node_to_concepts:
            foo = amr[FAKE_ROOT_VAR]
            amr.node_to_concepts[FAKE_ROOT_VAR] = FAKE_ROOT_CONCEPT
    ...
    def print_config(self, column_len=80):
        ...
        if self.cidx:
            if self.idx == START_ID:
                ...
                span_g = ','.join(tok['form'] for tok in GraphState.sent[self.idx:self.A.nodes[self.idx].end]) if isinstance(
                    self.idx, int) else self.A.nodes[self.idx].tag
            if self.cidx == START_ID:
                span_d = START_FORM
            else:
                ...
                span_d = ','.join(tok['form'] for tok in GraphState.sent[self.cidx:self.A.nodes[self.cidx].end]) if isinstance(
                    self.cidx, int) else self.A.nodes[self.cidx].tag
            ...
            if self.idx == START_ID:
                ...
                output += 'ID:%s %s\nParent:(%s-%s) Children:%s' % (str(GraphState.sentID), self.text,
                                                                    span_g, self.idx,
                                                                    ['(' + ','.join(tok['form'] for tok in GraphState.sent[c:self.A.nodes[c].end]) + ')' if isinstance(c, int) else '(' + self.A.nodes[c].tag + ')' for c in self.A.nodes[self.idx].children])
        ...
        for i in range(tnum):
            ...
            if i < num_p and i < num_r:
                ...
                gold_span_gg = ','.join(tok['form'] for tok in GraphState.sent[gg:self.gold_graph.nodes[gg].end]) if isinstance(
                    gg, int) else ','.join(self.gold_graph.nodes[gg].words)
                gold_span_gd = ','.join(tok['form'] for tok in GraphState.sent[gd:self.gold_graph.nodes[gd].end]) if isinstance(
                    gd, int) else ','.join(self.gold_graph.nodes[gd].words)
                parsed_span_g = ','.join(tok['form'] for tok in GraphState.sent[g:self.A.nodes[g].end]) if isinstance(
                    g, int) else ','.join(self.A.nodes[g].words)
                parsed_span_d = ','.join(tok['form'] for tok in GraphState.sent[d:self.A.nodes[d].end]) if isinstance(
                    d, int) else ','.join(self.A.nodes[d].words)
                ...
                parsed_tuple_str = "(%s(%s-%s:%s),(%s-%s:%s))" % (parsed_edge_label,
                                                                  parsed_span_g, g, parsed_tag_g, parsed_span_d, d, parsed_tag_d)
                ref_tuple_str = "(%s(%s-%s:%s),(%s-%s:%s))" % (gold_edge_label,
                                                               gold_span_gg, gg, gold_tag_gg, gold_span_gd, gd, gold_tag_gd)
    ...
    def write_basic_amr(self, out, CONST_REL='ARG0'):
        ...
        for g, d in parsed_tuples:
            ...
            if dep['head'] == gov['id']:  # tuple also in dependency tree
                rel = get_fake_amr_relation_mapping(dep['rel']) if get_fake_amr_relation_mapping(
                    dep['rel']) != 'NONE' else CONST_REL
                fake_amr_triples.append((rel, gov['lemma'], dep['lemma']))
            else:
                fake_amr_triples.append(
                    (CONST_REL, gov['lemma'], dep['lemma']))
