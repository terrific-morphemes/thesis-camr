...
from constants import DET_T2G_ORACLE_ABT, DET_T2G_ORACLE_ABT_ALIGN, ACTION_TYPE_TABLE
from constants import NEXT2, START_ID, START_EDGE
from constants import ACTION_WITH_EDGE, ACTION_WITH_TAG
...
class Parser(object):
    ...
    def __init__(self, model=None, oracle_type=DET_T2G_ORACLE_ABT, action_type='basic', verbose=1, elog=sys.stdout):
        self.sent = ''
        ...
        if self.oracle_type == DET_T2G_ORACLE_ABT:
        ...
        elif self.oracle_type == DET_T2G_ORACLE_ABT_ALIGN:
            Parser.State = __import__("graphstate2").GraphState
            Parser.State.init_action_table(ACTION_TYPE_TABLE[action_type])
            Parser.oracle = __import__("oracle2").DetOracleABT(self.verbose)
        else:
    ...
    def parse_corpus_train(self, instances, interval=500):
        ...
        for i, inst in enumerate(instances, 1):
            ...
            if i % interval == 0:
                ...
                print("Over " + str(i) + " sentences ",
                      "Accuracy:%s" % (p), file=self.elog)

        print("One pass on %s instances takes %s" % (str(i), datetime.timedelta(
            seconds=round(time.time() - start_time, 0))), file=self.elog)
    ...
    def parse_corpus_test(self, instances, EVAL=False):
        ...
        if EVAL:
            ...
            for i, inst in enumerate(instances, 1):
                ...
                print("Done parsing sentence %s" %
                      (state.sentID), file=self.elog)

            print("Parsing on %s instances takes %s" % (str(i), datetime.timedelta(
                seconds=round(time.time() - start_time, 0))), file=self.elog)
            p = n_correct_total / n_parsed_total if n_parsed_total != .0 else .0
            r = n_correct_total / n_gold_total
            f = 2 * p * r / (p + r)
            print("Unlabeled Precision:%s Recall:%s F1:%s" %
                  (p, r, f), file=self.elog)

            lp = n_correct_labeled_total / n_parsed_total
            lr = n_correct_labeled_total / n_gold_total
            lf = 2 * lp * lr / (lp + lr)
            print("Labeled Precision:%s Recall:%s F1:%s" %
                  (lp, lr, lf), file=self.elog)
        ...
        while not state.is_terminal():
            ...
            if len(actions) == 1:
                ...
                if train:
                    ...
                    scores = map(state.get_score,
                                 (act['type'] for act in actions), features)

                    best_act_ind, best_label_index = self.get_best_act(
                        scores, actions)  # ,argset)
                    best_act = actions[best_act_ind]
                    best_label = Parser.get_index_label(
                        best_act, best_label_index)

                    # print "Done argmax, %s"%(round(time.time()-start_time,2))
                    #gold_act = getattr(self,self.oracle_type)(state,ref_graph)
                    gold_act, gold_label = Parser.oracle.give_ref_action(
                        state, ref_graph)
                    ...
                    try:
                        ...
                        if self.verbose > 2:
                            print('WARNING: gold action %s not in possible action set %s' % (
                    ...
                    # update constraint
                    # if ('tag' in gold_act and gold_act['tag'] == NULL_TAG) \
                    #    or gold_label in [NULL_EDGE,NULL_TAG]:
                    #     violated = True

                    gold_label_index = Parser.get_label_index(
                        gold_act, gold_label)
                    '''
                    ...
                    if (gold_act != best_act or gold_label != best_label) and not violated:
                        self.perceptron.update_weight_one_step(
                            actions[gold_act_ind]['type'], features[gold_act_ind], gold_label_index, actions[best_act_ind]['type'], features[best_act_ind], best_label_index)
                    ...
                    if self.verbose > 3:  # for infer action, this may not work properly during decoding
                        gold_act, gold_label = Parser.oracle.give_ref_action(
                            state, ref_graph)
                        self.evaluate_actions(
                            actions[best_act_ind], best_label_index, gold_act, gold_label, ref_graph)
                        if self.verbose > 1:
                            print("Step %s: (%s,%s) | take action %s, label:%s | gold action %s,label:%s | State:sigma:%s beta:%s" % (
                                step, actions[best_act_ind]['type'], gold_act['type'], actions[best_act_ind], best_label, gold_act, gold_label, state.sigma, state.beta), file=sys.stderr)

                    if self.verbose > 0:
                        # correct next2 tag error
                        if best_act['type'] == NEXT2:
                            # self.output_weight(best_act_ind,best_label_index,features,actions)
                            gold_node = ref_graph.nodes.get(state.idx, None)
                            # and not (isinstance(gold_act['tag'],(ETag,ConstTag)) or re.match('\w+-\d+',gold_act['tag'])):
                            if gold_node and best_act.get('tag', None) != gold_node.tag:
                                print("Gold concept tag %s, Predicted concept tag %s " % (gold_node.tag.encode(
                                    'utf8'), best_act.get('tag', 'None').encode('utf8')), file=sys.stderr)
                    ...
                    if self.verbose > 5:
                        self.output_weight(
                            best_act_ind, best_label_index, features, actions)
                        if gold_act in actions:
                            gold_act_ind = actions.index(gold_act)
                            gold_label_index = Parser.State.model.rel_codebook.get_index(
                                gold_label)
                            self.output_weight(
                                gold_act_ind, gold_label_index, features, actions)
            ...
            if self.verbose == 2:
                ...
                print(best_act, gold_act, file=sys.stderr)
    ...
    def output_weight(self, act_ind, label_index, feats, actions):
        ...
        act_idx = GraphState.model.class_codebook.get_index(
            actions[act_ind]['type'])
        weight = GraphState.model.avg_weight[act_idx]
        feat_idx = map(
            GraphState.model.feature_codebook[act_idx].get_index, feats_fired)
        weight_sum = np.sum(
            weight[[i for i in feat_idx if i is not None]], axis=0)
        #weight_fired = weight[[i for i in feat_idx if i is not None]]
        try:
            print('\n'.join('%s,%f' % (f, weight[i][label_ind]) if i is not None else '%s,%f' % (
    ...
    def testOracleGuide(self, instance, start_step=0, concept_counter=None, act_counter=None):
        """
        simulate the oracle's action sequence
            concept_counter: dist for concept
            act_counter: dist for action types
        """
        ...
        while not state.is_terminal():
            ...
            if self.verbose > 0:
                ...
                if DRAW_GRAPH:
                    ...
                    self.draw_graph(fname, state.A.getPGStyleGraph(
                        (state.idx, state.cidx)))
            ...
            if self.verbose > 0:
                action_str = '{type: %s, tag: %s}' % (action['type'], action['tag'].encode(
                    'utf8')) if 'tag' in action else str(action)
                label_str = label.encode('utf8') if label else label
                status_str = "Step %s:take action %s, edge label %s | State:sigma:%s beta:%s" % (
                    step, action_str, label_str, state.sigma, state.beta)
                print(red + status_str + ending_color, file=sys.stderr)
                if action['type'] == 6:  # swap
                    raw_input('ENTER')
            ...
            if state.is_permissible(action):
                ...
                if concept_counter is not None and action['type'] == NEXT2 and 'tag' in action:
                    concept_counter[action['tag']] += 1

                if act_counter is not None:
                    act_counter[action['type']] += 1
            ...
            if state.is_permissible(action):
                if action['type'] == NEXT1:
                    if label != None and label != START_EDGE:
                        edge_label = state.A.get_edge_label(
                            state.idx, state.cidx)
                        if edge_label != label:
                            error_stat['edge_error']['edge_label_error'][edge_label].append(
                                state.sentID)
                    ...
                    tag = state.get_current_node().tag
                    error_stat['node_error']['extra_node_error'][tag].append(
                        state.sentID)
                    ...
                    bpos = GraphState.sent[state.cidx]['pos'] if isinstance(
                        state.cidx, int) else btag
                    brel = GraphState.sent[state.cidx]['rel'] if isinstance(
                        state.cidx, int) else btag
                    ...
                    if isinstance(aid, int):
                        ...
                        error_stat['edge_error'][act_name][bpos +
                                                           brel + apos].append(state.sentID)
                    else:
                        apos = atag
                        error_stat['edge_error'][act_name][apos].append(
                            state.sentID)
                else:
                    tag = state.get_current_node().tag
                    pos = GraphState.sent[state.idx]['pos'] if isinstance(
                        state.idx, int) else tag
                    btag = state.A.nodes[state.cidx].tag
                    bpos = GraphState.sent[state.cidx]['pos'] if isinstance(
                        state.cidx, int) else btag
                    act_name = GraphState.action_table[action['type']]
                    error_stat['edge_error'][act_name][pos +
                                                       bpos].append(state.sentID)
